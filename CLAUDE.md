...
### Recent Changes
- Added `business transactions update` (PUT) and `patch` (PATCH) — the SDK has had `UpdateDraftTransactionWithResponse` / `PatchDraftTransactionWithResponse` since v0.1.0 with no CLI surface. Both take the same flag set as `create` plus `--document-order` (`id=position`), since `TransactionParams.Documents` is `[]BundleOrder` — update/patch *reorder* attached documents, they don't upload; uploading is still `documents add`.
- `--signers-file` takes a JSON array of signer objects for the API's `signers` list (max 10, each needs an email; both checked locally before the request). `--email` remains the primary `signer`, which the create endpoint requires — the file is additive, not a replacement.
- The shared builder is now `buildTransactionParams(cmd) business.TransactionParams`, used directly by update/patch. `create` needs `TransactionCreateParams`, a separate generated struct, so `toCreateParams` converts by JSON round-trip — the two have identical JSON tags for all 26 shared fields and differ only in `signer` being required and `documents` carrying uploads. `TestBuildTransactionParams_SurvivesCreateConversion` asserts every key survives, so a future spec change that diverges a shared field's type fails the build rather than silently dropping the value from create requests.
- `anyFlagChanged` in `cmd/root.go` gates the nested `signer` object: a patch that touches only `--expiry` must not send a signer built from zero values and blank out the real one.
- `business transactions create` now covers the whole top-level `TransactionCreateParams` surface via `registerTransactionParamFlags` / `applyTransactionParams` in `cmd/business.go` (same split as `registerSCIMUserFlags` / `buildSCIMUserBody`). This fixed a live bug: eleven flags — `middle-name`, `phone-number`, `message-to-signer`, `message-subject`, `activation-time`, `expiry`, `suppress-email`, `auth-requirement`, `require-secondary-photo-id`, `payer`, `external-id` — were registered but never read by `Run`, so they appeared in `--help` and were silently discarded.
- Two helpers in `cmd/root.go` back that: `boolFlagIfSet` (a bool is only sent when the user passed the flag, since an explicit `false` would override an organization default) and `enumFlag` (validates against the spec's enum and exits 1 listing the valid set, instead of letting the API return an opaque 400). `--draft` is the deliberate exception — it has always been sent explicitly, so omitting it still means "not a draft".
- `--document` is now repeatable (`StringSlice`) since the API takes a `documents` array; `--document x` alone is unchanged. Nested signer fields beyond the scalars, and multi-signer transactions, are still out of reach of a flag interface and need the API directly.
- `cmd/business_test.go` asserts every registered flag reaches the request body, and that unset flags are absent from the marshaled JSON — the regression that let the eleven dead flags go unnoticed.
- Upgraded to `proof-sdk-go v0.4.0` (spec regeneration, purely additive). No CLI code changes were needed: the new fields (`idv_use_case`, `recipient_details_config`, `allowed_notary_states`, OIDC `options.expiry` / `options.skip_landing_page`) are transaction params this CLI does not surface, in line with `create-transaction` exposing only a small convenience flag set. The new `canceled` transaction status and `transaction.canceled` webhook event need no change either — `--status` and `--events` are pass-through strings, not validated enums.
